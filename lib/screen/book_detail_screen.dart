...
class _BookDetailScreenState extends State<BookDetailScreen> {
  ...
  DateTime? borrowDate;
  DateTime? returnDate;
  final endpoint = 'http://localhost:8000/api/index.php';
  ...
  Future<Book> fetchBookDetails(int id) async {
    final response = await http.get(Uri.parse('https://stephen-king-api.onrender.com/api/book/$id'));
  ...
  Future<MyBook> fetchMyBook(int id) async {
    final response = await http.get(Uri.parse('$endpoint?action=is_borrowed&book_id=$id'));
    if (response.statusCode == 200) {
      final responseBody = json.decode(response.body);
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (!myBook.isBorrowed) ...[
                          ...
                          const SizedBox(height: 8),
                          const Text('Tanggal Kembali', style: TextStyle(fontSize: 16)),
                          DatePickerWidget(
                            onDateSelected: (date) {
                              setState(() {
                                returnDate = date;
                            ...
                            onPressed: () async {
                              if (borrowDate != null && returnDate != null) {
                                try {
                                  ...
                                  bookProvider.borrowBook(context, idLibrary!, book.id, book.title, borrowDate!, returnDate!);
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(content: Text('Meminjam Buku: ${book.title}')),
                                  );
                                } catch (error) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                          ...
                          const Text('Buku ini telah dipinjam.', style: TextStyle(fontSize: 16)),
                        ],
                      ],
                    ),
                  );
