...
class _MyBooksScreenState extends State<MyBooksScreen> {
  ...
  Future<List<MyBook>> fetchMyBooks() async {
    ...
    final response = await http.get(Uri.parse(
        'http://localhost:8000/api/index.php?action=my_books&id_library=$idLibrary'));
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text('Pinjam: ${myBook.borrowDate} - Kembali: ${myBook.returnDate}'),
                  trailing: myBook.isBorrowed
                      ? ElevatedButton(
                          onPressed: () async {
                            final prefs = await SharedPreferences.getInstance();
                            final idLibrary = prefs.getString('id_library');
                            bookProvider.returnBook(context, idLibrary, myBook.bookId);
                          },
